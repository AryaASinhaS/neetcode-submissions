class Solution {
    public int orangesRotting(int[][] grid) {
        ...
        int rowLength = grid.length, colLength = grid[0].length, count = -1;       
        ...
        for(int row = 0; row < rowLength; row++)
        {
            for(int col = 0; col < colLength; col++)
            {
                if(grid[row][col] == 2)
                {
                    ...
                    computeMinTimeTaken(grid, row, col, visited, time);
                }
            }
        }
    ...
    public void computeMinTimeTaken(int[][] grid, int row, int col, HashSet<String> visited, int[][] time)
    {
